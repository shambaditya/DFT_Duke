...
// Filename       : patterns/picorv32_power_stuck_serial.v
// Scan operation : SERIAL
// Idstamp        : 2024.4:f61e:7579:2:a588
// Fault          : STUCK
// Coverage       : 95.74(TC) 88.63(FC)
// Date           : Thu Nov 20 22:37:35 2025
//
// Begin_Verify_Section 
//   format            = Verilog 
//   top_module_name   = picorv32_picorv32_power_stuck_serial_v_ctl 
//   serial_flag       = ON 
...
module picorv32_picorv32_power_stuck_serial_v_ctl;
...
`ifdef VCD
    $sformat(_vcd_dump_file_name, "patterns/picorv32_power_stuck_serial.v.dump");
...
`ifdef UTVCD
    $sformat(_utvcd_dump_file_name, "patterns/picorv32_power_stuck_serial.v.dump");
    if(_change_out_path) begin 
...
`ifdef debussy
    $sformat(_fsdb_dump_file_name, "patterns/picorv32_power_stuck_serial.v.fsdb");
    if(_change_out_path) begin 
...
`ifdef QWAVE
    $sformat(_qwave_dump_file_name, "patterns/picorv32_power_stuck_serial.v.qwave.db");
    if(_change_out_path) begin 
...
initial begin 
   if(_change_path) begin 
     $sformat(_new_filename,"%0s/patterns/picorv32_power_stuck_serial.v.po.name",_new_path); 
     $display("Loading %0s\n", _new_filename ); 
     $readmemh(_new_filename,_nam_obus,308,0); 
   end 
   else begin
     $display("Loading patterns/picorv32_power_stuck_serial.v.po.name");
     $readmemh("patterns/picorv32_power_stuck_serial.v.po.name",_nam_obus,308,0);
   end 
end 
      ...
      if ((_msk_obus[_bit_count] === 1'b1) &&
        ...
        if (_write_DIAG_file == 1) begin
          if (_DIAG_file_header == 0) begin
            if ((_start_pat > -1) && (_end_pat > -1)) begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v_%0d_%0d.fail",
                       _start_pat, _end_pat);
            end
            else if (_start_pat > -1) begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v_%0d.fail",
                       _start_pat);
            end
            else if (_end_pat > -1) begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v__%0d.fail",
                       _end_pat);
            end
            else begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v.fail");
            end
            if(_change_out_path) begin 
            ...
            if(_change_out_path) begin 
              $fwrite(_diag_file, "// This File is simulation generated (%0s/patterns/picorv32_power_stuck_serial.v)\n", _new_out_path);
            end
            else begin
              $fwrite(_diag_file, "// This File is simulation generated (patterns/picorv32_power_stuck_serial.v)\n");
            end
        ...
        if (_write_MASK_file == 1) begin
          if (_MASK_file_header == 0) begin
            if ((_start_pat > -1) && (_end_pat > -1)) begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v_%0d_%0d.mask",
                       _start_pat, _end_pat);
            end
            else if (_start_pat > -1) begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v_%0d.mask",
                       _start_pat);
            end
            else if (_end_pat > -1) begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v__%0d.mask",
                       _end_pat);
            end
            else begin
              $sformat(_tmp_filename, "patterns/picorv32_power_stuck_serial.v.mask");
            end
            if(_change_out_path) begin 
              $sformat(_tmp_filename, "%0s/%0s", _new_out_path, _tmp_filename);
            end
...
//  $display("Reading checkpoint patterns/picorv32_power_stuck_serial.v.dat");
//  $restart("patterns/picorv32_power_stuck_serial.v.dat");
end
...
if ($value$plusargs("CONFIG=%0s", _cfg_file_name)) begin
  ...
  _cfg_file_name = "patterns/picorv32_power_stuck_serial.v.cfg";
end
...
if(_change_path) begin 
  $sformat(_new_filename,"%0s/patterns/picorv32_power_stuck_serial.v.cfg",_new_path); 
  _config_file = $fopen(_new_filename, "r");
end
else begin
  _config_file = $fopen("patterns/picorv32_power_stuck_serial.v.cfg", "r");
end
...
begin
   ...
   if (_fstat != -1) begin
         ...
         if ((_start_pat != -1) && !_in_range && (_vec_pat_count != -1) &&
             ...
             ((_pat_num + _vec_pat_count) < (_start_pat - 1))) begin
           _max_index = -1;
   ...
   begin
      ...
      if ((_start_pat != -1) && ((_start_pat != 0) || (!_in_testsetup)) &&
        ...
        if ((_pat_num >= (_start_pat - 1)) && !_in_range) begin
          _in_range = 1;
          _do_compare = 0;
          if (_in_chaintest) begin
               ...
               1'b1: begin // timeplate 1 - gen_tp1
                  ...
                  if ((_miscompare_limit)&&(_compare_fail>=_miscompare_limit)) begin
                    ...
                    if (_DIAG_file_header == 1) begin
                       ...
                       $fwrite(_diag_file, "total_cycles 3614\n");
...
//  $display("Writing checkpoint patterns/picorv32_power_stuck_serial.v.dat");
//  $save("patterns/picorv32_power_stuck_serial.v.dat");
