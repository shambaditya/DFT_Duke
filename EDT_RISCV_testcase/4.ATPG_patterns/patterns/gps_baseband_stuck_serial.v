...
// Filename       : patterns/gps_baseband_stuck_serial.v
...
//   format            = Verilog 
//   top_module_name   = picorv32_gps_baseband_stuck_serial_v_ctl 
//   serial_flag       = ON 
...
module picorv32_gps_baseband_stuck_serial_v_ctl;
...
`ifdef VCD
    $sformat(_vcd_dump_file_name, "patterns/gps_baseband_stuck_serial.v.dump");
...
`ifdef UTVCD
    $sformat(_utvcd_dump_file_name, "patterns/gps_baseband_stuck_serial.v.dump");
    if(_change_out_path) begin 
...
`ifdef debussy
    $sformat(_fsdb_dump_file_name, "patterns/gps_baseband_stuck_serial.v.fsdb");
    if(_change_out_path) begin 
...
`ifdef QWAVE
    $sformat(_qwave_dump_file_name, "patterns/gps_baseband_stuck_serial.v.qwave.db");
    if(_change_out_path) begin 
...
initial begin 
   if(_change_path) begin 
     $sformat(_new_filename,"%0s/patterns/gps_baseband_stuck_serial.v.po.name",_new_path); 
     $display("Loading %0s\n", _new_filename ); 
     $readmemh(_new_filename,_nam_obus,308,0); 
   end 
   else begin
     $display("Loading patterns/gps_baseband_stuck_serial.v.po.name");
     $readmemh("patterns/gps_baseband_stuck_serial.v.po.name",_nam_obus,308,0);
   end 
end 
      ...
      if ((_msk_obus[_bit_count] === 1'b1) &&
        ...
        if (_write_DIAG_file == 1) begin
          if (_DIAG_file_header == 0) begin
            if ((_start_pat > -1) && (_end_pat > -1)) begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v_%0d_%0d.fail",
                       _start_pat, _end_pat);
            end
            else if (_start_pat > -1) begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v_%0d.fail",
                       _start_pat);
            end
            else if (_end_pat > -1) begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v__%0d.fail",
                       _end_pat);
            end
            else begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v.fail");
            end
            if(_change_out_path) begin 
            ...
            if(_change_out_path) begin 
              $fwrite(_diag_file, "// This File is simulation generated (%0s/patterns/gps_baseband_stuck_serial.v)\n", _new_out_path);
            end
            else begin
              $fwrite(_diag_file, "// This File is simulation generated (patterns/gps_baseband_stuck_serial.v)\n");
            end
        ...
        if (_write_MASK_file == 1) begin
          if (_MASK_file_header == 0) begin
            if ((_start_pat > -1) && (_end_pat > -1)) begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v_%0d_%0d.mask",
                       _start_pat, _end_pat);
            end
            else if (_start_pat > -1) begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v_%0d.mask",
                       _start_pat);
            end
            else if (_end_pat > -1) begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v__%0d.mask",
                       _end_pat);
            end
            else begin
              $sformat(_tmp_filename, "patterns/gps_baseband_stuck_serial.v.mask");
            end
            if(_change_out_path) begin 
              $sformat(_tmp_filename, "%0s/%0s", _new_out_path, _tmp_filename);
            end
...
//  $display("Reading checkpoint patterns/gps_baseband_stuck_serial.v.dat");
//  $restart("patterns/gps_baseband_stuck_serial.v.dat");
end
...
if ($value$plusargs("CONFIG=%0s", _cfg_file_name)) begin
  ...
  _cfg_file_name = "patterns/gps_baseband_stuck_serial.v.cfg";
end
...
if(_change_path) begin 
  $sformat(_new_filename,"%0s/patterns/gps_baseband_stuck_serial.v.cfg",_new_path); 
  _config_file = $fopen(_new_filename, "r");
end
else begin
  _config_file = $fopen("patterns/gps_baseband_stuck_serial.v.cfg", "r");
end
...
//  $display("Writing checkpoint patterns/gps_baseband_stuck_serial.v.dat");
//  $save("patterns/gps_baseband_stuck_serial.v.dat");
